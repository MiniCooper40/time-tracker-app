...
const Calender = <T,>({
  ...
  const calenderDays: CalenderDay[] = useMemo(
    () =>
  ...
  const horizontalPositionForDayRectTooltip = ({ rect }: CalenderDay) =>
    rect.x < layout.width / 2 ? "right" : "left";
        ...
        <Canvas style={{ flex: 1 }} onLayout={updateCanvasDimensions}>
          <Group>
            {calenderDays.map((day) => (
              <Box
                box={day.rect}
                key={`${day.rect.x}-${day.rect.y}`}
                color={colorForDay(day)}
                opacity={day === selectedDay ? 0.6 : 1}
              />
            ))}
            ...
            {selectedDay && (
              <SkTooltip
                ...
                positionVertical={verticalPositionForDayRectTooltip(
                  selectedDay,
                )}
                positionHorizontal={horizontalPositionForDayRectTooltip(
                  selectedDay,
                )}
