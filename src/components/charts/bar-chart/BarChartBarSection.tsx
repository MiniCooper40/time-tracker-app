...
const BarChartBarSection = ({sectionTop, sectionHeight, color="purple", pressed, opacity=1, currentBarHeight, barRect}: BarChartBarSectionProps) => {
    ...
    const derivedY = useDerivedValue(() => {
        ...
        const animatedRelSectionTop = relSectionTop * (currentBarHeight.value / barRect.height)
        return barBottom - animatedRelSectionTop
    })


    return <RoundedRect r={0} x={barRect.x} y={derivedY} width={barRect.width} height={derivedHeight} color={color} opacity={opacity} />
}
