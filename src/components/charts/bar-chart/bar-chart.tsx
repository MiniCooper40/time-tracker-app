import { Canvas, Group, Path, Skia, SkRect } from "@shopify/react-native-skia";
import { View } from "tamagui";
import { useMemo, useState } from "react";
import { BarChartBar } from "@/src/components/charts/bar-chart/bar-chart-bar";
import { LayoutChangeEvent } from "react-native";
import { BarChartLabel } from "@/src/components/charts/bar-chart/bar-chart-label";
import {
  Gesture,
  GestureDetector,
...
import { SkTooltip } from "@/src/components/charts/sk-tooltip";
...
const BarChart = ({
  ...
  const calculatedData: CalculatedBarChartEntry[] = useMemo(
    () =>
  ...
  const maxTick = useMemo(
    () => Math.max(...ticks.map(({ value }) => value)),
    [ticks],
  );
  const maxValue = useMemo(
    () => Math.max(...calculatedData.map(({ totalValue }) => totalValue)),
    [calculatedData],
  );
  ...
  const barCenters = useMemo(() => {
    return Array(calculatedData.length)
      .fill(0)
      .map((_, i) => {
  ...
  const overflowTopCorrection = useMemo(() => {
    const topLabelY =
      (tickCenters.at(-1) ?? 0) - (labelFont?.measureText("Label").height ?? 0);
  ...
  const labelY = useMemo(
    () => chartY + labelPadding + 20,
    [chartY, tooltipTitleFont],
  );
  const handlePress = (clickX: number, clickY: number) => {
    const clickBuffer = 3;
    barPositions.forEach(({ width, height, x, y }, index) => {
      if (
        clickX + clickBuffer >= x &&
        clickX - clickBuffer <= x + width &&
        clickY + clickBuffer >= y &&
        clickY - clickBuffer <= y + height
      ) {
        ...
        <Canvas style={{ flex: 1 }} onLayout={updateCanvasDimensions}>
          {layout.width !== 0 &&
            ...
            tooltipTitleFont && (
              <Group>
                {!loading &&
                  barPositions.map((position, barIndex) => (
                    <BarChartBar
                      key={`${barIndex}`}
                      x={position.x}
                      y={position.y}
                ...
                {labelFont &&
                  tickCenters.map((value, index) => {
                    const labelRect = labelFont.measureText(
                      ticks[index].label.toString(),
                    );
                ...
                {pressedBarIndex !== undefined && (
                  <SkTooltip
                    positionHorizontal={
                      pressedBarIndex > calculatedData.length / 2
                        ? "left"
                        : "right"
                    }
