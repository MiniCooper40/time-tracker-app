...
const SkTooltip = ({
  ...
  const entryRects = useMemo(
    () => entries.map((text) => entryFont.measureText(text)),
    [entries, entryFont],
  );

  const tooltipRect = useMemo(() => {
    const tooltipWidth =
      Math.max(titleRect.width, ...entryRects.map(({ width }) => width)) +
      padding * 2;
    const tooltipHeight =
      titleRect.height +
      padding * 2 +
      ...
      entryRects.length * entrySpacing;
    const tooltipX = (() => {
      switch (positionHorizontal) {
        ...
        case "left":
          return (
            containerRect.x - tooltipWidth - distanceFromCenter - arrowHeight
          );
      }
    })();
    const tooltipY = (() => {
  ...
  const directionAdjustedArrowVertices = useMemo(() => {
    if (positionHorizontal === "left") return arrowVertices;
      ...
      return arrowVertices.map(({ x, y }) =>
        vec(tooltipCenterX - (x - tooltipCenterX), y),
      );
    }
